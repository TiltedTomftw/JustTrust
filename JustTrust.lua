...
local function load_trust_sets_from_file()
    local file = io.open(settings_file, 'r')
    if file then
        local json_content = file:read('*all')
        file:close()
        
        if json_content and json_content:trim() ~= '' then
            -- Parse JSON content
            local success, parsed_data = pcall(function() return windower.from_json(json_content) end)
...
local function save_trust_sets_to_file()
    ...
    if file then
        local json_content = windower.to_json(trust_sets)
